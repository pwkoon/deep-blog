...
const PostCard = ({post, username, handleClick, handleDelete, handleEdit}: Props) => {
    ...
    <div className="bg-font-sand p-3 max-w-sm rounded overflow-hidden shadow-lg">
      ...
      <div className="px-6 py-4">
        ...
        <div className="text-center">
          ...
          {
            post.user.username === username ? 
          <button className="bg-button-delete text-white p-3 m-2" onClick={() => handleDelete && handleDelete(post)}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
          ...
          <p className='text-end'><b>{post.created_at}</b> — <i>{post.user.username}</i></p>
        </div>
      </div>
    </div>
  )
